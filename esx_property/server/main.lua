...
function SetPropertyOwned(name, price, rented, owner, type)
	MySQL.Async.execute('INSERT INTO owned_properties (name, price, rented, owner, type) VALUES (@name, @price, @rented, @owner, @type)',
	{
		...
		if xPlayer then
			...
			if rented then
				if type == 'motel' then
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('rented_motel', ESX.Math.GroupDigits(price)))
				else
...
function RemoveOwnedProperty(name, owner)
	...
	MySQL.Async.execute('DELETE FROM owned_properties WHERE name = @name AND owner = @owner',
	{
		...
		if xPlayer then
			...
			TriggerClientEvent('esx:showNotification', xPlayer.source, _U('made_motel'))
			end
		end
	end)
end
...
ESX.RegisterServerCallback('esx_propertys:requestMotelRooms', function(source, cb)
	local xPlayer  = ESX.GetPlayerFromId(source)
	MySQL.Async.fetchAll('SELECT * FROM owned_properties WHERE type = @type AND owner = @owner', { ['@owner'] = xPlayer.identifier, ['@type'] = 'motel' }, function(result)
		if result[1] ~= nil then
		cb(GetProperty(result[1].name))
		else
		cb(nil)
		end
	end)
...
AddEventHandler('esx_property:rentProperty', function(propertyName)
	...
	if property.motelroom == 'motel' then
		rent = Config.MotelRentalPrice
	end
	SetPropertyOwned(propertyName, rent, true, xPlayer.identifier, property.motelroom)
end)
...
AddEventHandler('esx_property:buyProperty', function(propertyName)
	...
	if property.price <= xPlayer.getMoney() then
		...
		SetPropertyOwned(propertyName, property.price, false, xPlayer.identifier, property.motelroom)
	else
